...
Item {
    ...
    property int slideshowInterval: slideshowState.interval
    readonly property var _intervalList: [5, 10, 30, 60, 300, 600, 1800, 3600]

    function getIntervalLabel(val) {
    ...
    property string currentNotification: {
        ...
        if (window.currentFilter === "All") return window.isSlideshow ? "Slideshow ▶" : "";
    ...
    function triggerOnlineSearch() {
        ...
        const cmd = `
            ...
            for p in $(pgrep -f '[d]dg_search.sh'); do
                if [ "$p" != "$$" ] && [ "$p" != "$BASHPID" ]; then
    ...
    function stepToNextValidIndex(direction) {
        ...
        let filterOrder = ["All", "Red", "Orange", "Yellow", "Green", "Blue", "Purple", "Pink", "Monochrome"];
    ...
    Rectangle {
        ...
        Row {
            ...
            // Standalone slideshow toggle button (▶ / ⏸)
            Rectangle {
                id: slideshowBtn
                height: 44
                width: 44
                radius: 10
                anchors.verticalCenter: parent ? parent.verticalCenter : undefined
                color: window.isSlideshow ? _theme.surface2 : "transparent"
                border.color: window.isSlideshow ? _theme.text : Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.6)
                border.width: window.isSlideshow ? 2 : 1

                Behavior on color { ColorAnimation { duration: 300 } }
                Behavior on border.color { ColorAnimation { duration: 300 } }
                scale: window.isSlideshow ? 1.15 : (slideshowMouse.containsMouse ? 1.08 : 1.0)
                Behavior on scale { NumberAnimation { duration: 400; easing.type: Easing.OutBack; easing.overshoot: 1.2 } }

                Canvas {
                    width: 44; height: 44
                    anchors.centerIn: parent
                    property bool running: window.isSlideshow
                    property string activeColor: window.isSlideshow ? _theme.text : (slideshowMouse.containsMouse ? _theme.text : Qt.rgba(_theme.text.r, _theme.text.g, _theme.text.b, 0.7))
                ...
                MouseArea {
                    ...
                    onClicked: {
                        if (window.isSlideshow) {
                            window.isSlideshow = false
                            Quickshell.execDetached(["bash", "-c", "pkill -f slideshow.sh || true; notify-send -i preferences-desktop-wallpaper 'Slideshow off' -t 2000"])
                        } else {
                            window.isSlideshow = true
                            Quickshell.execDetached(["bash", "-c", "nohup bash ~/.config/hypr/scripts/slideshow.sh " + window.slideshowInterval + " > /dev/null 2>&1 & notify-send -i preferences-desktop-wallpaper 'Slideshow active' -t 2000"])
                        }
                    }
                }
            }

            // Interval cycling pill
            Rectangle {
                id: slideshowIntervalBtn
                height: 44
                width: window.isSlideshow ? (intervalLabel.implicitWidth + 36) : 0
                visible: width > 0.1
                radius: 10
                clip: true
                color: Qt.rgba(_theme.surface2.r, _theme.surface2.g, _theme.surface2.b, 0.5)
                border.color: Qt.rgba(_theme.surface1.r, _theme.surface1.g, _theme.surface1.b, 0.6)
                border.width: 1
                anchors.verticalCenter: parent ? parent.verticalCenter : undefined

                Behavior on width { NumberAnimation { duration: 400; easing.type: Easing.OutBack; easing.overshoot: 0.5 } }

                Text {
                    id: intervalLabel
                    anchors.centerIn: parent
                    text: window.getIntervalLabel(window.slideshowInterval)
                    color: _theme.text
                ...
                MouseArea {
                    ...
                    cursorShape: Qt.PointingHandCursor
                    onClicked: {
                        let current = window.slideshowInterval;
                        let idx = window._intervalList.indexOf(current);
                        if (idx === -1) {
                            idx = 0;
                            for (let i = 0; i < window._intervalList.length; i++) {
                                if (window._intervalList[i] >= current) { idx = i; break; }
                            }
                        }
                        let nextIdx = (idx + 1) % window._intervalList.length;
                        let nextVal = window._intervalList[nextIdx];
                        
                        slideshowState.interval = nextVal;
                        window.slideshowInterval = nextVal;
                        
                        if (window.isSlideshow) {
                            Quickshell.execDetached(["bash", "-c", 
                                "pkill -f slideshow.sh || true; sleep 0.2; nohup bash ~/.config/hypr/scripts/slideshow.sh " + nextVal + " > /dev/null 2>&1 &"]);
                        }
                    }
                }
            }
    ...
    Process {
        id: slideshowStatusCheck
        command: ["bash", "-c", "pgrep -f '[s]lideshow.sh' > /dev/null && echo 1 || echo 0"]
        stdout: StdioCollector {
            onStreamFinished: {
                window.isSlideshow = this.text.trim() === "1"
            }
        }
    }

    Component.onCompleted: {
        slideshowStatusCheck.running = true
    ...
    Component.onDestruction: {
        if (window.hasSearched) {
            ...
            Quickshell.execDetached(["bash", "-c", "echo 'stop' > /tmp/ddg_search_control; for p in $(pgrep -f '[d]dg_search.sh'); do if [ \"$p\" != \"$$\" ] && [ \"$p\" != \"$BASHPID\" ]; then kill -9 $p 2>/dev/null || true; fi; done; pkill -f '[g]et_ddg_links.py'"]);
        }
    }
}
